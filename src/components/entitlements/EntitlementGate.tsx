...
const COPY: Partial<Record<MerchantFeature, { icon: string; title: string; body: string }>> = {
  ...
  webhooks: {
    icon: "webhook",
    title: "Outbound webhooks are not on your plan",
    body:
      "Push every new lead, booked meeting and sent quote to your own CRM as a signed HTTP " +
...
export function EntitlementGate({ feature, children, title, description, icon }: Props) {
  ...
  if (!allowed(feature)) {
    // "Your plan does not include this" and "this was switched off for you" are different
    // sentences, and only `overridden` can tell them apart. Support cannot help without it.
    const reason = isOverride(feature)
      ? "Switched off for this workspace by Frostrek — contact support."
      : entitlements?.plan_name
        ? `Current plan: ${entitlements.plan_name}`
        : undefined;
    return (
      <PageState
        icon={icon || copy?.icon || "lock"}
        title={title || copy?.title || "Not on your plan"}
        description={
